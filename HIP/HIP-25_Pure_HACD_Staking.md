...
| `STAKING_FEE_SHARE` | `40%` | share of eligible fees routed to reward pool (both sources) |
...
|---|---|---|
| HACD inscription **protocol fee** (HIP-15) | Burned | `40%` → staking pool; `60%` → burn (unchanged effective burn reduction) |
| HACD transfer tx fee | `90%` burned | `40%` of total transfer fee → staking pool; `60%` follows existing burn/miner split |
...
| Parameter | Decision |
|---|---|
| Fee redirect | `40%` on inscription protocol fees **and** `40%` on HACD transfer fees |
