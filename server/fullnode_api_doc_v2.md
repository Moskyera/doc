...
The standard HTTP interface request and response method is adopted, including 4 paths:
   ...
   1. /create generates data, mostly GET method (such as creating an account or transfer); some endpoints use POST (such as `/create/transaction`).
   2. /submit submits data, mostly POST method (such as `/submit/transaction`); some miner endpoints use GET (such as `/submit/miner/success`).
...
| coinkind | menu |-| h, s, d, a, hs, hd, hsd, hsda, all, a(1,2,3) | Filter returned account and transaction information types. `h`: hacash, `s`: satoshi, `d`: diamond, `a`: assets. `a(1,2,3)` means only include specific asset serial numbers. |
...
#### 1.1 Create an account `GET: /create/account`
...
```js
{
    ...
    hash: "b563b525d5a840f14c2f1d7da52a1eb3f7ee7357e784af8b68d0b0f74b95cbb4" // submitted tx hash
}
```
...
| ---- | ---- | ---- | ---- |
| balance | address, unit, coinkind, diamonds, assets, asset | address | up to 200 comma-delimited addresses |
...
| coin/transfer | height, txposi, coinkind, unit, from, to, filter_from, filter_to | height + txposi | coinkind supports `h/s/d/a`, `all`, `hsda`, and `a(1,2,3)` |
| transaction | hash, unit| hash| return transaction info and actions |

Current route map in latest implementation (2026-02-13):

- server common APIs: `GET /query/latest`, `GET /query/coin/transfer`, `GET /create/account`, `GET /create/coin/transfer`
- mint APIs: `GET /`, `GET /query/block/intro`, `GET /query/block/recents`, `GET /query/block/views`, `GET /query/block/datas`, `GET /query/balance`, `GET /query/diamond`, `GET /query/diamond/bidding`, `GET /query/diamond/views`, `GET /query/diamond/engrave`, `GET /query/diamond/inscription_protocol_cost`, `GET /query/transaction`, `GET /query/supply`, `GET /query/fee/average`, `GET /query/hashrate`, `GET /query/hashrate/logs`, `GET /query/channel`, `GET /query/miner/pending`, `GET /query/miner/notice`, `GET /query/diamondminer/init`, `GET /submit/miner/success`, `POST /submit/diamondminer/success`, `POST /create/transaction`, `POST /submit/transaction`, `POST /submit/block`, `POST /operate/fee/raise`, `POST /util/transaction/check`, `POST /util/transaction/sign`
- vm APIs (when `vm` feature is enabled): `GET /query/contract/sandboxcall`, `GET /query/contract/logs`, `GET /operate/contract/logs/delete`
...
- coinkind [menu] : Query the return type; `coinkind=h` only returns HAC balance, `coinkind=hs` returns HAC + BTC balance, and default `coinkind=hsd` returns all basic coin types
- assets [bool] : Optional, whether to return all asset balances
- asset [uint64] : Optional, return only one asset serial balance
...
```js
{
    ...
    born: {
        height: 179610, // Height of excavated block
        hash: "000000001e76b27f70f9a5e5b0f9b9824b416c210548ffad298577ed5378cdcd"
    },
...
{
    ret: 0,
    height: 575611, // latest block height in effective blockchain
}
```
...
- coinkind [menu] Optional, query transfer type; `coinkind=h` returns only HAC transfers, `coinkind=hs` returns HAC and BTC transfers, default `coinkind=hsda` returns all (including assets)
...
- filter_from [string] Optional, alias of `from`
- filter_to [string] Optional, alias of `to`
...
```js
{
    ...
    main_address: "13RnDii79ypWayV8XkrBFFci29cHtzmq3Z",
    transfers: [
        {
...
- hash [string] Tx hash (hex string)
...
```js
{
    ...
    burned_fee: 890984.44654182, // Tx fee by burning
...
- unit [string] Optional, whether to return floating-point string in units of "mei" or "248"
- burn90 [bool] Optional, if true fee estimate will be multiplied by 10 for burn-90% scenarios
...
{
    ret: 0,
    purity: 5020, // average fee purity (unit-238)
...
#### 4.1 Increase transaction fee for confirmation `POST: /operate/fee/raise`
...
- hash [string] The hash value of the transaction to increase the fee
...
[http://nodeapi.hacash.org/operate/fee/raise?fee_prikey=8D969EEF6ECAD3C29A3A629280E686CF0C3F5D5A86AFF3CA12020C923ADC6C92&fee=32:247&hash=ad26a35116664176426f3c08adad147577b9a85999cb89d465becf6a27002c04](http://nodeapi.hacash.org/operate/fee/raise?fee_prikey=8D969EEF6ECAD3C29A3A629280E686CF0C3F5D5A86AFF3CA12020C923ADC6C92&fee=32:247&hash=ad26a35116664176426f3c08adad147577b9a85999cb89d465becf6a27002c04)
...
- stuff [bool] Optional, Whether to return the full internal mining stuff object
